class Profiler:
    ...
    def get_model_size(self, model, opt=None):
        if opt is None:
            self._logger.info("[OPTION: ALL] #parameters: %.4fM", sum(p.numel() for p in model.parameters()) / 1e6)
        else:
            ...
            if opt == "trainable-only":
                ...
                if hasattr(model, "blocks"):
                    self._logger.info(
                        "[OPTION: %s] #parameters: %.4fM", opt, sum(p.numel() for p in model.blocks.parameters()) / 1e6
                    )
