...
from .configs import TransINRNerfConfig
...
def volume_rendering(raw_outputs, z_values):
    """
...
class TransINRNerf(nn.Module):
    Config = TransINRNerfConfig

    def __init__(self, config: TransINRNerfConfig):
        super().__init__()
        self.config = config = config.copy()  # type: TransINRNerfConfig
    ...
    def forward(
        ...
        outputs = raw_outputs
        if return_rgb:
            outputs = volume_rendering(outputs, z_values)
